...
def is_new_frame_better(source, driving, predictor):
    ...
    if puppet_kp is None:
        display_string = "No face detected in puppet."
        return False
    ...
    if new_kp is not None:
        new_norm = (np.abs(puppet_kp - new_kp) ** 2).sum()
        old_norm = (np.abs(puppet_kp - predictor.get_start_frame_kp()) ** 2).sum()
...
def load_images(IMG_SIZE = 256):
    puppets = []
    filenames = []
    images_list = sorted(glob.glob(f'{opt.puppets}/*'))
    for i, f in enumerate(images_list):
        if f.endswith('.jpg') or f.endswith('.jpeg') or f.endswith('.png'):
            ...
            puppets.append(img)
            filenames.append(f)
    return puppets, filenames

def change_puppet(predictor, new_puppet):
    global puppet, puppet_kp, kp_source
    puppet_kp = predictor.get_frame_kp(new_puppet)
    kp_source = None
    puppet = new_puppet
    predictor.set_source_image(puppet)
...
def print_help():
    info('\n\n=== Control keys ===')
    info('1-9: Change Puppet')
    for i, fname in enumerate(Puppet_names):
        key = i + 1
        name = fname.split('/')[-1]
        info(f'{key}: {name}')
    info('W: Zoom camera in')
    info('S: Zoom camera out')
    info('A: Previous Puppet in folder')
    info('D: Next Puppet in folder')
    info('Q: Get random Puppet')
    info('X: Calibrate face pose')
    info('I: Show FPS')
    info('ESC: Quit')
    info('\nFull key list: https://github.com/msafdarkhan/merapuppet#controls')
    info('\n\n')
...
if __name__ == "__main__":
    ...
    puppets, puppet_names = load_images()
    ...
    cur_ava = 0    
    puppet = None
    change_puppet(predictor, puppets[cur_ava])
    passthrough = False
    ...
    try:
        while True:
            ...
            if find_keyframe:
                if is_new_frame_better(puppet, frame, predictor):
            ...
            if key == 27: # ESC
                ...
                if not is_calibrated:
                    cv2.namedWindow('puppetify', cv2.WINDOW_GUI_NORMAL)
                    cv2.moveWindow('puppetify', 600, 250)
                ...
                try:
                    log('Loading StyleGAN puppet...')
                    puppet = load_stylegan_puppet()
                    passthrough = False
                    change_puppet(predictor, puppet)
                except:
                    log('Failed to load StyleGAN puppet')
            elif key == ord('l'):
                try:
                    log('Reloading puppets...')
                    puppets, puppet_names = load_images()
                    passthrough = False
                    log("Images reloaded")
                except:
                ...
                cur_ava = min(key - 49, len(puppets) - 1)
                passthrough = False
                change_puppet(predictor, puppets[cur_ava])
            elif key == 48:
            ...
            if overlay_alpha > 0:
                preview_frame = cv2.addWeighted( puppets[cur_ava], overlay_alpha, frame, 1.0 - overlay_alpha, 0.0)
            else:
